...
export async function insertShiftEntry(data) {
  ...
  if (selectedDate.getTime() === today.getTime()) {
    ...
    if (data.shift === 'sabah') {
      ...
      // Öğle: 01:30'a kadar (gece yarısı sonrası) zamanında
      if (hour === 0 || hour === 1) {
        // 00:00 - 01:30 arası zamanında
        if (hour === 0) {
          isOnTime = true  // 00:00 - 00:59 her zaman zamanında
        } else if (hour === 1 && minute <= 30) {
          isOnTime = true  // 01:00 - 01:30 zamanında
        } else {
          isOnTime = false // 01:31+ geç
        }
      } else if (hour >= 16 && hour <= 23) {
        // 16:00 - 23:59 arası (vardiya saatleri) zamanında kabul et
        isOnTime = true
      } else {
        // Gündüz saatleri (02:00 - 15:59) geç
        isOnTime = false
      }
    }
  } else {
  ...
  // ÖĞLE VARDİYASI İÇİN SABAH VERİSİNİ ÇIKAR
  let actualRevenue = {
  ...
  // Eğer öğle vardiyası ise sabah verisini çıkar
  if (data.shift === 'ogle') {
