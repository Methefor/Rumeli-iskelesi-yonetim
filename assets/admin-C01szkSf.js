import"./modulepreload-polyfill-B5Qt9EMX.js";import{supabase as h}from"./supabase-client-CFSZQWn8.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";let v="month",w=null,x=null,k=null;function E(e){return e.getFullYear()+"-"+String(e.getMonth()+1).padStart(2,"0")+"-"+String(e.getDate()).padStart(2,"0")}async function Z(){let e,a=null;const t=new Date;if(v==="day"&&k)e=k,a=k;else if(v==="custom"&&w&&x)e=w,a=x;else switch(v){case"week":const l=new Date;l.setDate(l.getDate()-7),e=E(l);break;case"month":e=E(new Date(t.getFullYear(),t.getMonth(),1)),a=E(new Date(t.getFullYear(),t.getMonth()+1,0));break;case"lastmonth":e=E(new Date(t.getFullYear(),t.getMonth()-1,1)),a=E(new Date(t.getFullYear(),t.getMonth(),0));break;case"all":e="2020-01-01";break;default:e=E(new Date(t.getFullYear(),t.getMonth(),1)),a=E(new Date(t.getFullYear(),t.getMonth()+1,0))}let i=h.from("daily_reports").select("*, cashiers(name)").gte("date",e).order("date",{ascending:!1});a&&(i=i.lte("date",a));const[s,n]=await Promise.all([i,h.from("cashiers").select("*").order("total_points",{ascending:!1})]);return{weekly:[],reports:s.data||[],cashiers:n.data||[]}}function W(e){const a={};return e.forEach(t=>{const i=`${t.date}-${t.kasa}`;(!a[i]||t.shift==="aksam")&&(a[i]=t)}),Object.values(a).reduce((t,i)=>(t.restoran+=parseFloat(i.rumeli_z2||0),t.cafetarya+=parseFloat(i.rumeli_z1||0),t.balik+=parseFloat(i.balik_ekmek||0),t.dondurma+=parseFloat(i.dondurma||0),t.total+=parseFloat(i.rumeli_z2||0)+parseFloat(i.rumeli_z1||0)+parseFloat(i.balik_ekmek||0)+parseFloat(i.dondurma||0),t),{restoran:0,cafetarya:0,balik:0,dondurma:0,total:0})}function z(e){const a={};e.forEach(s=>{const n=`${s.date}-${s.kasa}`;(!a[n]||s.shift==="aksam")&&(a[n]=s)});const t={};Object.values(a).forEach(s=>{const n=s.date;t[n]||(t[n]=0),t[n]+=parseFloat(s.rumeli_z2||0)+parseFloat(s.rumeli_z1||0)+parseFloat(s.balik_ekmek||0)+parseFloat(s.dondurma||0)});const i=Object.keys(t).sort();return{labels:i.map(s=>K(s)),data:i.map(s=>t[s])}}function U(e){const a={};e.forEach(i=>{const s=`${i.date}-${i.kasa}`;(!a[s]||i.shift==="aksam")&&(a[s]=i)});const t={sabah:0,aksam:0};return Object.values(a).forEach(i=>{const s=parseFloat(i.rumeli_z2||0)+parseFloat(i.rumeli_z1||0)+parseFloat(i.balik_ekmek||0)+parseFloat(i.dondurma||0);i.shift==="sabah"?t.sabah+=s:i.shift==="aksam"&&(t.aksam+=s)}),t}function Q(e,a){const t={};return e.forEach(i=>{var n;const s=((n=i.cashiers)==null?void 0:n.name)||"Bilinmiyor";t[s]||(t[s]=0),t[s]+=parseFloat(i.individual_revenue||0)}),t}function X(e,a){const t=new Date().toISOString().split("T")[0],i=e.filter(d=>d.date===t),s=[...new Set(i.map(d=>d.cashier_id))],n=a.filter(d=>!s.includes(d.id)),l=e.filter(d=>!d.is_on_time),g=z(e),r=Math.max(...g.data,0);return{notEntered:n,lateEntries:l,maxCiro:r}}function J(e){const a=z(e),t=Math.max(...a.data,0),i=new Date().toISOString().split("T")[0],s=e.filter(d=>d.date===i),l=[...new Set(s.map(d=>d.cashier_id))].length;return{maxDailyCiro:t,activeCashiers:l,avgEntryTime:"2.5dk",weeklyGrowth:"+12%"}}function aa(){const e=new Date,a=e.getDay(),t=["2026-01-01","2026-04-23","2026-05-01","2026-05-19","2026-08-30","2026-10-29"],i=e.toISOString().split("T")[0];return t.includes(i)?"special":a===0||a===6?"weekend":"weekday"}async function ea(){try{const e=aa();console.log("Hedef tipi:",e);const{data:a,error:t}=await h.from("targets").select("amount").eq("target_type",e).single();return t?(console.error("Hedef çekme hatası:",t),{weekday:15e4,weekend:2e5,special:25e4}[e]):(console.log("Çekilen hedef:",a.amount),parseFloat(a.amount)||15e4)}catch(e){return console.error("fetchDailyTarget hatası:",e),15e4}}function ta(){typeof anime>"u"||(anime({targets:".kasa-card, .sc, .total-hero-card",opacity:[0,1],translateY:[40,0],delay:anime.stagger(100),duration:800,easing:"easeOutCubic"}),anime({targets:".rank-card",opacity:[0,1],translateX:[-40,0],delay:anime.stagger(120),duration:700,easing:"easeOutCubic"}))}function ia(){if(typeof anime>"u")return;const e=document.getElementById("totalDailyCiro"),a=parseFloat(e.getAttribute("data-value")||0);anime({targets:{value:0},value:a,duration:2e3,easing:"easeOutExpo",update:function(s){e.textContent=m(s.animations[0].currentValue)}});const t=document.getElementById("progressBarFill"),i=parseFloat(t.getAttribute("data-percent")||0);anime({targets:"#progressBarFill",width:i+"%",duration:1500,easing:"easeInOutQuad"})}function sa(e){const a={gida:{name:"Gıda",icon:"🥗",total:0,color:"#22c55e"},kahvalti:{name:"Kahvaltı",icon:"🥐",total:0,color:"#f59e0b"},kahve:{name:"Kahve",icon:"☕",total:0,color:"#8b5cf6"},meyvesuyu:{name:"Meyve Suyu",icon:"🧃",total:0,color:"#ec4899"},sicak_icecek:{name:"Sıcak İçecek",icon:"🔥",total:0,color:"#ef4444"},soguk_icecek:{name:"Soğuk İçecek",icon:"🥤",total:0,color:"#3b82f6"},tatli:{name:"Tatlı",icon:"🍰",total:0,color:"#a855f7"}},t={};e.forEach(n=>{const l=`${n.date}-${n.kasa}`;(!t[l]||n.shift==="aksam")&&(t[l]=n)}),Object.values(t).forEach(n=>{a.gida.total+=parseFloat(n.gida)||0,a.kahvalti.total+=parseFloat(n.kahvalti)||0,a.kahve.total+=parseFloat(n.kahve)||0,a.meyvesuyu.total+=parseFloat(n.meyvesuyu)||0,a.sicak_icecek.total+=parseFloat(n.sicak_icecek)||0,a.soguk_icecek.total+=parseFloat(n.soguk_icecek)||0,a.tatli.total+=parseFloat(n.tatli)||0});const i=Object.values(a).reduce((n,l)=>n+l.total,0);Object.values(a).forEach(n=>{n.percentage=i>0?n.total/i*100:0});const s=Object.entries(a).map(([n,l])=>({key:n,...l})).sort((n,l)=>l.total-n.total);return{categories:s,total:i,highest:s[0],lowest:s[s.length-1],average:i/s.length}}function na(e){return e.categories.map(a=>`
            ...
            `).join("")}function oa(e){var t;const a=((t=e.categories[0])==null?void 0:t.total)||1;return e.categories.map(i=>{const s=a>0?i.total/a*100:0;return`
            ...
            `}async function ra(e,a,t){const i=await ea(),s=W(a),n=z(a),l=U(a),g=Q(a),r=X(a,t),d=J(a),y=sa(a),f={};a.forEach(o=>{var B;const c=o.cashier_id,R=((B=o.cashiers)==null?void 0:B.name)||"Bilinmiyor";f[c]||(f[c]={name:R,points:0,entries:0,revenue:0}),f[c].points+=parseInt(o.points_earned)||0,f[c].entries+=1,f[c].revenue+=parseFloat(o.individual_revenue)||0});const $=Object.values(f).sort((o,c)=>c.points-o.points),b=$.length>0?$[0]:null,C=a.length,S=a.filter(o=>o.is_on_time).length,P=C>0?Math.round(S/C*100):0,_=[...new Set(a.map(o=>o.date))].length,H=_>0?s.total/_:0,j=s.total,q=v==="day"&&k?k:new Date().toISOString().split("T")[0],I={};a.filter(o=>o.date===q).forEach(o=>{const c=`${o.date}-${o.kasa}`;(!I[c]||o.shift==="aksam")&&(I[c]=o)});const L=Object.values(I).reduce((o,c)=>o+parseFloat(c.total_revenue||0),0),D=i>0?Math.min(100,Math.round(L/i*100)):0;document.getElementById("mainContent").innerHTML=`
            <!-- HERO -->
            ...
            <div class="total-hero-card">
                <div class="total-hero-left">
                    ...
                    <div class="total-hero-amount" id="totalDailyCiro" data-value="${j}">0,00 ₺</div>
            ...
            <div class="daily-target-card">
                ...
                <div class="target-body">
                    ...
                    <div class="target-current-section">
                        ...
                        <div class="target-current">${m(L)}</div>
                    </div>
                </div>
            ...
            <div id="rankGrid" class="rank-grid">
                ${$.map((o,c)=>{const R=t.find(N=>(N.name||"")===o.name)||{},B=R.id||"",V=Y(R.badge_level);return`
                    <div class="rank-card">
                        <div class="rank-pos p${Math.min(c+1,4)}">#${c+1}</div>
                        <div class="rank-avatar av-${ua(o.name)}">${ma(o.name)}</div>
                        <div class="rank-info">
                        ...
                        <div class="rank-right">
                            ...
                            <div class="rank-badge-pill">${V}</div>
                            ${B?`<button class="badge-assign-btn" onclick="assignBadge('${B}','${o.name}')">🎖️ Rozet Ver</button>`:""}
                        </div>
                    </div>`}).join("")}
            </div>
            ...
            <div class="summary-grid">
                <div class="sc blue">
                    ...
                    <div class="sc-s">${_} günlük veri</div>
                </div>
                <div class="sc purple">
                    <div class="sc-l">Günlük Ortalama</div>
                    <div class="sc-v">${u(H)}</div>
                    <div class="sc-s">Son ${_} gün</div>
                </div>
                <div class="sc orange">
                    <div class="sc-l">Zamanında Giriş</div>
                    <div class="sc-v">%${P}</div>
                    <div class="sc-s">${S}/${C} giriş</div>
                </div>
                <div class="sc green">
                ...
                <div class="alert-card ${r.lateEntries.length>0?"warning":"ok"}">
                    ...
                    ${r.lateEntries.length===0?'<div class="alert-item">Bu hafta geç giriş yok! 🎉</div>':r.lateEntries.slice(0,5).map(o=>{var c;return`
                            <div class="alert-item">
                                ...
                                <span style="color:var(--orange)">${O(o.entry_time)}</span>
                            </div>
                        `}).join("")}
                </div>
        ...
        `,da(n,s,l,g);const M=document.getElementById("applyFiltersBtn"),G=document.getElementById("clearFiltersBtn");M&&M.addEventListener("click",ca),G&&G.addEventListener("click",va),document.getElementById("lastUpdate").textContent="Son güncelleme: "+new Date().toLocaleTimeString("tr-TR"),setTimeout(()=>{ta(),ia()},100),setTimeout(()=>{const o=document.getElementById("dailyProgressBar");o&&(o.style.width=Math.min(100,parseFloat(o.dataset.percent))+"%")},200)}let p={};function da(e,a,t,i){Chart.defaults.color="#7a8ba8",Chart.defaults.borderColor="#1e3a5f",Chart.defaults.font.family="DM Sans";const s=document.getElementById("dailyChart");s&&(p.daily&&p.daily.destroy(),p.daily=new Chart(s,{type:"bar",data:{labels:e.labels.length>0?e.labels:["Veri Yok"],datasets:[{label:"Günlük Ciro (₺)",data:e.data.length>0?e.data:[0],backgroundColor:"rgba(59,130,246,.7)",borderColor:"#3b82f6",borderWidth:2,borderRadius:8}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{x:{grid:{color:"#1e3a5f"}},y:{grid:{color:"#1e3a5f"},ticks:{callback:r=>"₺"+new Intl.NumberFormat("tr-TR").format(r)}}}}}));const n=document.getElementById("kasaChart");if(n){p.kasa&&p.kasa.destroy();const r=a.total>0,d=r?[a.restoran,a.cafetarya,a.balik,a.dondurma]:[.001,.001,.001,.001];p.kasa=new Chart(n,{type:"doughnut",data:{labels:["Restoran","Cafetarya","Balık Ekmek","Dondurma"],datasets:[{data:d,backgroundColor:["rgba(59,130,246,.8)","rgba(139,92,246,.8)","rgba(6,182,212,.8)","rgba(236,72,153,.8)"],borderColor:"#111d32",borderWidth:3,hoverOffset:8}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"bottom",labels:{padding:16,usePointStyle:!0}},tooltip:{enabled:r,callbacks:{label:y=>{const f=y.raw,$=y.dataset.data.reduce((C,S)=>C+S,0),b=Math.round(f/$*100);return` ${y.label}: ${m(f)} (%${b})`}}}},cutout:"65%"}})}const l=document.getElementById("shiftChart");l&&(p.shift&&p.shift.destroy(),p.shift=new Chart(l,{type:"bar",data:{labels:["Sabah Vardiyası","Akşam Vardiyası"],datasets:[{label:"Ciro (₺)",data:[t.sabah||0,t.aksam||0],backgroundColor:["rgba(245,158,11,.7)","rgba(139,92,246,.7)"],borderColor:["#f59e0b","#8b5cf6"],borderWidth:2,borderRadius:8}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{x:{grid:{color:"#1e3a5f"}},y:{grid:{color:"#1e3a5f"},ticks:{callback:r=>"₺"+new Intl.NumberFormat("tr-TR").format(r)}}}}}));const g=document.getElementById("cashierChart");if(g){p.cashier&&p.cashier.destroy();const r=Object.keys(i),d=Object.values(i);p.cashier=new Chart(g,{type:"bar",data:{labels:r.length>0?r:["Veri Yok"],datasets:[{label:"Ciro (₺)",data:d.length>0?d:[0],backgroundColor:["rgba(139,92,246,.7)","rgba(245,158,11,.7)","rgba(236,72,153,.7)","rgba(34,197,94,.7)"],borderColor:["#8b5cf6","#f59e0b","#ec4899","#22c55e"],borderWidth:2,borderRadius:8}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{x:{grid:{color:"#1e3a5f"}},y:{grid:{color:"#1e3a5f"},ticks:{callback:y=>"₺"+new Intl.NumberFormat("tr-TR").format(y)}}}}})}}let F=[];function A(e){return e.length===0?'<tr><td colspan="9" style="text-align:center;color:var(--txt2);padding:40px;">Henüz veri yok</td></tr>':e.map(a=>{var s;const t=parseFloat(a.total_revenue||0),i=parseFloat(a.individual_revenue||t);return`
                <tr>
                    ...
                    <td class="mono">${O(a.entry_time)}</td>
                    <td class="r">
            ...
            `}).join("")}function ca(){const e=document.getElementById("filterDate").value,a=document.getElementById("filterKasiyer").value,t=document.getElementById("filterKasa").value,i=document.getElementById("filterVardiya").value;let s=[...F];e&&(s=s.filter(n=>n.date===e)),a&&(s=s.filter(n=>n.cashier_id===a)),t&&(s=s.filter(n=>n.kasa===t)),i&&(s=s.filter(n=>n.shift===i)),document.getElementById("entriesBody").innerHTML=A(s)}function va(){document.getElementById("filterDate").value="",document.getElementById("filterKasiyer").value="",document.getElementById("filterKasa").value="",document.getElementById("filterVardiya").value="",document.getElementById("entriesBody").innerHTML=A(F)}function m(e){return new Intl.NumberFormat("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2}).format(e||0)+" ₺"}function u(e){return e>=1e6?(e/1e6).toFixed(1)+"M ₺":e>=1e3?(e/1e3).toFixed(1)+"K ₺":m(e)}function K(e){return e?new Date(e).toLocaleDateString("tr-TR",{day:"2-digit",month:"2-digit"}):"-"}function O(e){return e?new Date(e).toLocaleTimeString("tr-TR",{hour:"2-digit",minute:"2-digit"}):"-"}function ma(e){return e?e.split(" ").map(a=>a[0]).join(""):"??"}function ua(e){if(!e)return"t";const a=e.split(" ")[0];return{Tuba:"t",Elif:"e",Melda:"m",Ceren:"c"}[a]||"t"}function Y(e){return{yeni:"🌱 Yeni",bronz:"🥉 Bronz",gumus:"🥈 Gümüş",altin:"🥇 Altın",elmas:"💎 Elmas",efsane:"👑 Efsane",efsane_plus:"👑⭐ Süper Efsane",ozel_yildiz:"🌟 Yılın Yıldızı",ozel_ates:"🔥 Ateş Çıkışlı",ozel_roket:"🚀 Hızlı Yükseliş",ozel_onur:"🏅 Onur Madalyası",ozel_takim:"🤝 Takım Ruhu"}[e]||"🌱 Yeni"}window.assignBadge=async function(e,a){const t=[{key:"ozel_yildiz",icon:"🌟",name:"Yılın Yıldızı",desc:"En parlak performans"},{key:"ozel_ates",icon:"🔥",name:"Ateş Çıkışlı",desc:"Haftanın en iyisi"},{key:"ozel_roket",icon:"🚀",name:"Hızlı Yükseliş",desc:"En hızlı gelişim"},{key:"ozel_onur",icon:"🏅",name:"Onur Madalyası",desc:"Olağanüstü hizmet"},{key:"ozel_takim",icon:"🤝",name:"Takım Ruhu",desc:"Ekip çalışması"}],i=document.createElement("div");i.className="badge-modal-overlay",i.innerHTML=`
