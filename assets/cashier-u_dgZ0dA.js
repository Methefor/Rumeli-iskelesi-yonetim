import"./modulepreload-polyfill-B5Qt9EMX.js";import{supabase as f,updateShiftEntry as C}from"./supabase-client-Cz_bCqFO.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";typeof window._cashierLoadOk=="function"&&window._cashierLoadOk();const S=new URLSearchParams(window.location.search),v=S.get("cashier_id");v?D():(x("Kasiyer ID'si bulunamadı. Lütfen giriş sayfasından tekrar deneyin."),document.getElementById("loading").style.display="none");function b(e){return e.getFullYear()+"-"+String(e.getMonth()+1).padStart(2,"0")+"-"+String(e.getDate()).padStart(2,"0")}async function D(){try{const e=new Date,t=b(new Date(e.getFullYear(),e.getMonth(),1)),a=b(new Date(e.getFullYear(),e.getMonth()+1,0)),[o,i,s,r]=await Promise.all([f.from("cashiers").select("*").eq("id",v).single(),f.from("daily_reports").select("*").eq("cashier_id",v).gte("date",t).lte("date",a),f.from("daily_reports").select("cashier_id, points_earned, is_on_time, rumeli_z1, total_revenue, cashiers(name)").gte("date",t).lte("date",a),f.from("entry_history").select("*").eq("cashier_id",v).order("entry_time",{ascending:!1}).limit(10)]);if(o.error)throw o.error;const d=o.data,n=i.data||[],c=s.data||[],u=r.data||[],y=n.reduce((l,m)=>l+(parseInt(m.points_earned)||0),0),g=n.length,B={entries:g,onTime:n.filter(l=>l.is_on_time).length,revenue:n.reduce((l,m)=>l+(parseFloat(m.individual_revenue)||0),0),points:y,performancePct:g>0?Math.round(y/(g*100)*100):0},p={};c.forEach(l=>{var k;const m=l.cashier_id,w=((k=l.cashiers)==null?void 0:k.name)||"Bilinmiyor";p[m]||(p[m]={cashier_id:m,cashier_name:w,total_score:0,total_entries:0,on_time_entries:0});const $=(l.is_on_time?50:0)+(parseFloat(l.rumeli_z1||0)>0||parseFloat(l.total_revenue||0)>0?50:0);p[m].total_score+=$,p[m].total_entries+=1,l.is_on_time&&(p[m].on_time_entries+=1)});const I=Object.values(p).map(l=>({...l,performance_pct:l.total_entries>0?Math.round(l.total_score/(l.total_entries*100)*100):0})).sort((l,m)=>m.performance_pct!==l.performance_pct?m.performance_pct-l.performance_pct:m.total_entries-l.total_entries);T(d,B,I,u),await _(v),document.getElementById("loading").style.display="none",document.getElementById("dashboard").style.display="block"}catch(e){console.error("Dashboard yükleme hatası:",e),x("Dashboard yüklenirken bir hata oluştu: "+e.message),document.getElementById("loading").style.display="none"}}function T(e,t,a,o){var u;const i={"Tuba Bozaklı":"av-t","Elif Yıldırım":"av-e","Melda Yılmaz":"av-m","Ceren Erdem":"av-c"},s=e.name.split(" ").map(y=>y[0]).join("");document.getElementById("profileAvatar").textContent=s,document.getElementById("profileAvatar").className="avatar "+(i[e.name]||"av-t"),document.getElementById("profileName").textContent=e.name,document.getElementById("profileRole").textContent="Kasiyer — Rumeli İskelesi";const r=t.performancePct,d=M(e.badge_level,r);document.getElementById("badgeIcon").textContent=d.icon,document.getElementById("badgeLabel").textContent=d.label,document.getElementById("totalPoints").textContent=t.entries>0?`%${r}`:"—",document.querySelector('[style*="puan"]')&&((u=document.querySelector('[style*="color: var(--txt2)"]'))==null?void 0:u.textContent)===" puan"&&(document.querySelector('[style*="color: var(--txt2)"]').textContent=" performans");const n=L(r,e.badge_level);document.getElementById("nextBadge").textContent=n.label,document.getElementById("pointsNeeded").textContent=n.pctLeft>0?`%${n.pctLeft} daha`:"Tamamlandı!",document.getElementById("progressFill").style.width=Math.min(r,100)+"%",document.getElementById("statEntries").textContent=t.entries,document.getElementById("statOnTime").textContent=t.onTime,document.getElementById("statRevenue").textContent=h(t.revenue);const c=document.querySelectorAll(".stat-label");c[0]&&(c[0].textContent="Bu Ay Giriş"),R(a.slice(0,4),v),F(o)}function M(e,t){const a={ozel_yildiz:{icon:"🌟",label:"Yılın Yıldızı"},ozel_ates:{icon:"🔥",label:"Ateş Çıkışlı"},ozel_roket:{icon:"🚀",label:"Hızlı Yükseliş"},ozel_onur:{icon:"🏅",label:"Onur Madalyası"},ozel_takim:{icon:"🤝",label:"Takım Ruhu"}};return e&&a[e]?a[e]:t>=95?{icon:"🦈",label:"Efsane Kaptan"}:t>=85?{icon:"🌊",label:"Deniz Kurdu"}:t>=75?{icon:"🏛️",label:"İskele Reisi"}:t>=65?{icon:"⛵",label:"Kıdemli Kaptan"}:t>=50?{icon:"🧭",label:"Lostromo"}:t>=30?{icon:"🪢",label:"Güverte Tayfası"}:{icon:"⚓",label:"Stajyer Tayfa"}}function L(e,t){return t!=null&&t.startsWith("ozel_")?{label:"Özel Rozet (Müdür)",pctLeft:0}:e>=95?{label:"Maksimum Seviye — Tam Deniz Kurdu!",pctLeft:0}:e>=85?{label:"Efsane Kaptan 🦈",pctLeft:95-e}:e>=75?{label:"Deniz Kurdu 🌊",pctLeft:85-e}:e>=65?{label:"İskele Reisi 🏛️",pctLeft:75-e}:e>=50?{label:"Kıdemli Kaptan ⛵",pctLeft:65-e}:e>=30?{label:"Lostromo 🧭",pctLeft:50-e}:{label:"Güverte Tayfası 🪢",pctLeft:30-e}}function R(e,t){const a=document.getElementById("leaderboard");a.innerHTML="";const o={"Tuba Bozaklı":"av-t","Elif Yıldırım":"av-e","Melda Yılmaz":"av-m","Ceren Erdem":"av-c"};e.forEach((i,s)=>{const r=s+1,d=i.cashier_id===t,n=i.cashier_name?i.cashier_name.split(" ").map(g=>g[0]).join(""):"??",c=i.performance_pct??0,u=c>=85?"var(--green)":c>=65?"var(--orange)":"var(--txt2)",y=document.createElement("div");y.className="leaderboard-item"+(d?" current":""),y.innerHTML=`
                ...
                `,t.appendChild(o)})}function h(e){return new Intl.NumberFormat("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2}).format(e)+"₺"}function x(e){const t=document.getElementById("error");t.textContent="❌ "+e,t.style.display="block"}function z(e,t="success"){const a=document.createElement("div");a.style.cssText=`
            ...
            `,a.textContent=e,document.body.appendChild(a),setTimeout(()=>a.remove(),3e3)}function K(e){return new Date(e+"T00:00:00").toLocaleDateString("tr-TR",{day:"2-digit",month:"2-digit"})}async function _(e){try{const t=b(new Date),a=new Date;a.setDate(a.getDate()-7);const o=b(a),{data:i,error:s}=await f.from("daily_reports").select("*").eq("cashier_id",e).gte("date",o).order("date",{ascending:!1}).order("entry_time",{ascending:!1});if(s)throw s;const r=i.filter(n=>n.date===t),d=i.filter(n=>n.date!==t);A(r,d)}catch(t){console.error("Girişleri çekme hatası:",t)}}function A(e,t){const a=document.getElementById("todayEntriesContainer"),o=document.getElementById("pastEntriesContainer");a.innerHTML=e.length===0?`<div style="text-align:center;padding:20px;color:var(--txt2);">
                ...
                <div class="entry-card" data-entry-id="${e.id}">
                    <div class="entry-header">
                        <div>
                            <div style="font-weight:700;margin-bottom:4px;">${K(e.date)}</div>
