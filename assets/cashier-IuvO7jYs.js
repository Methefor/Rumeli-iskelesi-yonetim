import"./modulepreload-polyfill-B5Qt9EMX.js";import{supabase as f,updateShiftEntry as w}from"./supabase-client-9tBk9xhh.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";const S=new URLSearchParams(window.location.search),y=S.get("cashier_id");y?D():(x("Kasiyer ID'si bulunamadı. Lütfen giriş sayfasından tekrar deneyin."),document.getElementById("loading").style.display="none");function b(e){return e.getFullYear()+"-"+String(e.getMonth()+1).padStart(2,"0")+"-"+String(e.getDate()).padStart(2,"0")}async function D(){try{const e=new Date,t=b(new Date(e.getFullYear(),e.getMonth(),1)),a=b(new Date(e.getFullYear(),e.getMonth()+1,0)),[o,i,s,r]=await Promise.all([f.from("cashiers").select("*").eq("id",y).single(),f.from("daily_reports").select("*").eq("cashier_id",y).gte("date",t).lte("date",a),f.from("daily_reports").select("cashier_id, points_earned, is_on_time, rumeli_z1, total_revenue, cashiers(name)").gte("date",t).lte("date",a),f.from("entry_history").select("*").eq("cashier_id",y).order("entry_time",{ascending:!1}).limit(10)]);if(o.error)throw o.error;const d=o.data,n=i.data||[],c=s.data||[],u=r.data||[],v=n.reduce((l,m)=>l+(parseInt(m.points_earned)||0),0),g=n.length,I={entries:g,onTime:n.filter(l=>l.is_on_time).length,revenue:n.reduce((l,m)=>l+(parseFloat(m.individual_revenue)||0),0),points:v,performancePct:g>0?Math.round(v/(g*100)*100):0},p={};c.forEach(l=>{var k;const m=l.cashier_id,$=((k=l.cashiers)==null?void 0:k.name)||"Bilinmiyor";p[m]||(p[m]={cashier_id:m,cashier_name:$,total_score:0,total_entries:0,on_time_entries:0});const C=(l.is_on_time?50:0)+(parseFloat(l.rumeli_z1||0)>0||parseFloat(l.total_revenue||0)>0?50:0);p[m].total_score+=C,p[m].total_entries+=1,l.is_on_time&&(p[m].on_time_entries+=1)});const z=Object.values(p).map(l=>({...l,performance_pct:l.total_entries>0?Math.round(l.total_score/(l.total_entries*100)*100):0})).sort((l,m)=>m.performance_pct!==l.performance_pct?m.performance_pct-l.performance_pct:m.total_entries-l.total_entries);M(d,I,z,u),await _(y),document.getElementById("loading").style.display="none",document.getElementById("dashboard").style.display="block"}catch(e){console.error("Dashboard yükleme hatası:",e),x("Dashboard yüklenirken bir hata oluştu: "+e.message),document.getElementById("loading").style.display="none"}}function M(e,t,a,o){var u;const i={"Tuba Bozaklı":"av-t","Elif Yıldırım":"av-e","Melda Yılmaz":"av-m","Ceren Erdem":"av-c"},s=e.name.split(" ").map(v=>v[0]).join("");document.getElementById("profileAvatar").textContent=s,document.getElementById("profileAvatar").className="avatar "+(i[e.name]||"av-t"),document.getElementById("profileName").textContent=e.name,document.getElementById("profileRole").textContent="Kasiyer — Rumeli İskelesi";const r=t.performancePct,d=T(e.badge_level,r);document.getElementById("badgeIcon").textContent=d.icon,document.getElementById("badgeLabel").textContent=d.label,document.getElementById("totalPoints").textContent=t.entries>0?`%${r}`:"—",document.querySelector('[style*="puan"]')&&((u=document.querySelector('[style*="color: var(--txt2)"]'))==null?void 0:u.textContent)===" puan"&&(document.querySelector('[style*="color: var(--txt2)"]').textContent=" performans");const n=L(r,e.badge_level);document.getElementById("nextBadge").textContent=n.label,document.getElementById("pointsNeeded").textContent=n.pctLeft>0?`%${n.pctLeft} daha`:"Tamamlandı!",document.getElementById("progressFill").style.width=Math.min(r,100)+"%",document.getElementById("statEntries").textContent=t.entries,document.getElementById("statOnTime").textContent=t.onTime,document.getElementById("statRevenue").textContent=h(t.revenue);const c=document.querySelectorAll(".stat-label");c[0]&&(c[0].textContent="Bu Ay Giriş"),R(a.slice(0,4),y),F(o)}function T(e,t){const a={ozel_yildiz:{icon:"🌟",label:"Yılın Yıldızı"},ozel_ates:{icon:"🔥",label:"Ateş Çıkışlı"},ozel_roket:{icon:"🚀",label:"Hızlı Yükseliş"},ozel_onur:{icon:"🏅",label:"Onur Madalyası"},ozel_takim:{icon:"🤝",label:"Takım Ruhu"}};return e&&a[e]?a[e]:t>=95?{icon:"👑⭐",label:"Süper Efsane"}:t>=85?{icon:"👑",label:"Efsane"}:t>=75?{icon:"💎",label:"Elmas"}:t>=65?{icon:"🥇",label:"Altın"}:t>=50?{icon:"🥈",label:"Gümüş"}:t>=30?{icon:"🥉",label:"Bronz"}:{icon:"🌱",label:"Yeni Başlayan"}}function L(e,t){return t!=null&&t.startsWith("ozel_")?{label:"Özel Rozet (Müdür)",pctLeft:0}:e>=95?{label:"Maksimum Seviye",pctLeft:0}:e>=85?{label:"Süper Efsane",pctLeft:95-e}:e>=75?{label:"Efsane",pctLeft:85-e}:e>=65?{label:"Elmas",pctLeft:75-e}:e>=50?{label:"Altın",pctLeft:65-e}:e>=30?{label:"Gümüş",pctLeft:50-e}:{label:"Bronz",pctLeft:30-e}}function R(e,t){const a=document.getElementById("leaderboard");a.innerHTML="";const o={"Tuba Bozaklı":"av-t","Elif Yıldırım":"av-e","Melda Yılmaz":"av-m","Ceren Erdem":"av-c"};e.forEach((i,s)=>{const r=s+1,d=i.cashier_id===t,n=i.cashier_name?i.cashier_name.split(" ").map(g=>g[0]).join(""):"??",c=i.performance_pct??0,u=c>=85?"var(--green)":c>=65?"var(--orange)":"var(--txt2)",v=document.createElement("div");v.className="leaderboard-item"+(d?" current":""),v.innerHTML=`
                ...
                `,a.appendChild(v)})}function F(e){const t=document.getElementById("historyBody");if(t.innerHTML="",e.length===0){t.innerHTML='<tr><td colspan="5" style="text-align: center; color: var(--txt2); padding: 40px">Henüz giriş kaydı yok</td></tr>';return}e.forEach(a=>{const o=document.createElement("tr"),i=new Date(a.entry_time),s=i.toLocaleDateString("tr-TR",{day:"2-digit",month:"short",year:"numeric",timeZone:"Europe/Istanbul"}),r=i.toLocaleTimeString("tr-TR",{hour:"2-digit",minute:"2-digit",timeZone:"Europe/Istanbul"}),d=a.shift==="sabah"?"Sabah":"Akşam",n=a.shift==="sabah"?"shift-sabah":"shift-aksam",c=a.is_on_time?"✅":"⚠️",u=a.is_on_time?"Zamanında":"Geç",v=a.is_on_time?"status-ontime":"status-late";o.innerHTML=`
                    <td>
